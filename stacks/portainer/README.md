...
```
spin up portainer           # http://localhost:8089
spin open portainer         # same thing, in your browser
spin logs portainer
```
...
change the password in the UI, or `spin destroy portainer` and start over.
...
down portainer` keeps it; `spin destroy portainer` deletes it and the next
...
- `portainer-init` is a one-shot BusyBox container that writes
  ...
  as `Exited (0)` in `spin ps`, which is what it should look like.
...
- Portainer sees every container on the daemon, including the other spinup
  stacks. Stopping one from the UI works exactly like `spin down`.
